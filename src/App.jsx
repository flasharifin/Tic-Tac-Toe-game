...
function deriveWinner(gameBoard, playerName) {
  ...
  for (const combination of WINNING_COMBINATIONS) {
    ...
    const twoSquareSymbol = gameBoard[combination[1].row][combination[1].column];
    const thirdSquareSymbol = gameBoard[combination[2].row][combination[2].column];


    if (firstSquareSymbol && firstSquareSymbol === twoSquareSymbol && firstSquareSymbol === thirdSquareSymbol) {
...
function App() {
  ...
  function handelPlayerActive(rowIndex, colIndex) {
    ...
    <main>
      <div id="game-container">
        <ol id="players" className="highlight-player">
          ...
          <Player initialName={PLAYERS.O} symbol="O" isActive={activePlayer === "O"} onChangeName={handleChangePlayerName} />
        </ol>
        {/* <GameBoard onSelectedSquare={handelPlayerActive} activePlayerSymbol={activePlayer} /> */}
        {/* {winner && <p>Winner is {winner}</p>} */}
        {(winner || hasDraw) && <GameOver winner={winner} onRestart={handleBtnPlayAgain} />}
        <GameBoard onSelectedSquare={handelPlayerActive} board={gameBoard} />
      </div>
      <Log turns={gameTurns} />
    </main>
  )
